...
async def my_scheduled_task():
    try:
        ...
        for folder_path in folder_paths:
            file_dict[folder_path] = get_folder_id_from_path(folder_path=folder_path)  # Storing folder_id for efficient accessing  
            curr_files += [os.path.join(folder_path, file) for file in os.listdir(folder_path)] # Adding all files in the current folder

        image_file_paths = set(image_file_paths) 
        curr_files = set(curr_files)

        need_to_delete_files = image_file_paths.difference(curr_files) # {Image Paths Exists in Database} \ {curr_files}
        need_to_add_files = curr_files.difference(image_file_paths) # {curr_files} \ {Image Paths Exists in Database}
