...
async def run_get_classes(img_path: str) -> None:
...
async def test_route(payload: TestRouteRequest) -> TestRouteResponse:
    try:
        model_path: str = DEFAULT_FACE_DETECTION_MODEL
        yolov8_detector = YOLOv8(model_path, conf_thres=0.2, iou_thres=0.3)
        img_path: str = payload.path
...
@exception_handler_wrapper
def add_single_image(payload: AddSingleImageRequest) -> AddSingleImageResponse:
    try:
...
def test_images() -> TestImageResponse:
    try:
