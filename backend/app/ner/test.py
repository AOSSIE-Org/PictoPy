...
def ner_marking(text1):
    ...
    def perform_ner(text, session, tokenizer, id2label):
        ...
        combined_tokens, combined_labels = combine_subwords(
...
session = onnxruntime.InferenceSession(
    r"C:\Users\sanid\Downloads\gsoc_@pictopy\PictoPy\backend\app\models\facenet.onnx",
...
def scanned_embeddings(name):
    ...
    face_cascade = cv2.CascadeClassifier(
        r"C:\Users\sanid\Downloads\gsoc_@pictopy\PictoPy\backend\app\ner\haarcascade_frontalface_default.xml"
    )
    if face_cascade.empty():
        raise FileNotFoundError(
            "Failed to load Haar cascade file. Check the file path."
        )
