...
def db_get_all_images(tagged: Union[bool, None] = None) -> List[dict]:
    """
    Get all images from the database with their tags using optimized connection.
...
def db_get_untagged_images() -> List[UntaggedImageRecord]:
    """
    Find all images that need AI tagging using optimized connection.
...
def db_update_image_tagged_status(image_id: ImageId, is_tagged: bool = True) -> bool:
    """
    Update the isTagged status for a specific image using optimized connection.
...
def db_insert_image_classes_batch(image_class_pairs: List[ImageClassPair]) -> bool:
    """
    Insert multiple image-class pairs into the image_classes table using optimized connection.
...
def db_get_images_by_folder_ids(
    ...
    """
    Get all images that belong to the specified folder IDs using optimized connection.
...
def db_delete_images_by_ids(image_ids: List[ImageId]) -> bool:
    """
    Delete multiple images from the database by their IDs using optimized connection.
