...
class TestFoldersAPI:
    ...
    def test_add_folder_success(self, mock_folder_exists, mock_find_parent, 
                               mock_update_parent_ids, mock_add_folder_tree, 
                               client, temp_folder_structure):
        ...
        assert data["folder_id"] == "test-folder-id-123"
    ...
    @patch('app.routes.folders.db_find_parent_folder_id')
    @patch('app.routes.folders.db_folder_exists')
    def test_add_folder_with_parent_id(self, mock_folder_exists, mock_find_parent, 
                                      mock_update_parent_ids, mock_add_folder_tree, 
                                      client, temp_folder_structure):
        ...
        data = response.json()
        assert data["success"] is True
        assert data["folder_id"] == "child-folder-id"
    ...
    @patch('app.routes.folders.folder_util_add_folder_tree')
    @patch('app.routes.folders.db_folder_exists')
    def test_add_folder_database_error(self, mock_folder_exists, mock_add_folder_tree, 
                                      client, temp_folder_structure):
    ...
    @patch('app.routes.folders.db_find_parent_folder_id')
    @patch('app.routes.folders.db_folder_exists')
    def test_add_folder_background_processing_called(self, mock_folder_exists, mock_find_parent, 
                                                    mock_update_parent_ids, mock_add_folder_tree, 
                                                    client, temp_folder_structure):
    ...
    def test_enable_ai_tagging_partial_update(self, mock_enable_batch, client):
        ...
        request_data = {
            "folder_ids": ["existing-folder-1", "existing-folder-2", "non-existent-folder"]
        }
    ...
    @patch('app.routes.folders.db_folder_exists')
    @patch('app.routes.folders.db_enable_ai_tagging_batch')
    def test_complete_folder_workflow(self, mock_enable_batch, mock_folder_exists, 
                                     mock_find_parent, mock_update_parent_ids, 
                                     mock_add_folder_tree, client, temp_folder_structure):
        ...
        folder_id = add_response.json()["folder_id"]
        ...
        assert enable_response.json()["updated_count"] == 1
    ...
    @patch('app.routes.folders.db_find_parent_folder_id')
    @patch('app.routes.folders.db_folder_exists')
    def test_nested_folder_addition_workflow(self, mock_folder_exists, mock_find_parent, 
                                            mock_update_parent_ids, mock_add_folder_tree, 
                                            client, temp_folder_structure):
