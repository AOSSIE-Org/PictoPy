...
class TestFaceClustersAPI:
    ...
    def test_metadata_string_is_parsed(self, mock_get_cluster, mock_get_images):
        """Metadata stored as JSON string should be parsed into a dict."""
        mock_get_cluster.return_value = {"cluster_id": "cluster_123", "cluster_name": "John"}
        mock_get_images.return_value = [{
            "image_id": "img_1",
            "image_path": "/path/to/image1.jpg",
            "thumbnail_path": "/path/to/thumb1.jpg",
        ...
        assert image["metadata"]["camera"] == "Canon EOS"

    @patch("app.routes.face_clusters.db_get_images_by_cluster_id")
    @patch("app.routes.face_clusters.db_get_cluster_by_id")
    def test_metadata_dict_passes_through(self, mock_get_cluster, mock_get_images):
        """Metadata already as dict should stay unchanged."""
        mock_get_cluster.return_value = {"cluster_id": "cluster_123", "cluster_name": "John"}
        mock_get_images.return_value = [{
            "image_id": "img_1",
            "image_path": "/path/to/image1.jpg",
    ...
    def test_invalid_metadata_returns_none(self, mock_get_cluster, mock_get_images):
        """Broken JSON metadata should return None without crashing."""
        mock_get_cluster.return_value = {"cluster_id": "cluster_123", "cluster_name": "John"}
        mock_get_images.return_value = [{
            "image_id": "img_1",
            "image_path": "/path/to/image1.jpg",
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["data"]["images"][0]["metadata"] is None
