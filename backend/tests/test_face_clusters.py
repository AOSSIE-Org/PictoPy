...
def sample_clusters_with_counts():
    ...
    return [
        {"cluster_id": "cluster_1", "cluster_name": "John Doe", "face_count": 15},
        {"cluster_id": "cluster_2", "cluster_name": "Jane Smith", "face_count": 8},
        {"cluster_id": "cluster_3", "cluster_name": "Unknown Person", "face_count": 3},
    ]
...
class TestFaceClustersAPI:
    ...
    def test_rename_cluster_success(
        ...
        response = client.put(
            f"/face_clusters/{cluster_id}", json={"cluster_name": cluster_name}
        )
        ...
        assert (
            f"Successfully renamed cluster to '{cluster_name}'"
    ...
    def test_rename_cluster_not_found(self, mock_get_cluster):
        ...
        response = client.put(
            "/face_clusters/nonexistent_cluster", json={"cluster_name": "New Name"}
        )
    ...
    def test_rename_cluster_invalid_path(self, invalid_cluster_id):
        ...
        response = client.put(
            f"/face_clusters/{invalid_cluster_id}", json={"cluster_name": "Valid Name"}
        )
    ...
    def test_rename_cluster_name_whitespace_trimming(self, sample_cluster_data):
        ...
        with patch("app.routes.face_clusters.db_get_cluster_by_id") as mock_get, patch(
            "app.routes.face_clusters.db_update_cluster"
        ) as mock_update:
            ...
            mock_update.assert_called_once_with(
                cluster_id="cluster_123", cluster_name="John Doe"  # Should be trimmed
            )
