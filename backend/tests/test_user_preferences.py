...
class TestUserPreferencesAPI:
    ...
    def test_get_user_preferences_with_defaults(
        self, 
        mock_get_metadata, 
        sample_metadata_without_preferences
    ):
    ...
    @pytest.mark.parametrize("yolo_size,gpu_accel", [
        ("small", True),
        ("medium", False),
        ("nano", True),
        (None, False),
    ...
    def test_update_user_preferences_invalid_yolo_size(self):
        ...
        response = client.put(
            "/user_preferences/",
            json={"YOLO_model_size": "invalid_size"}
        )
