...
def setup_logging(component_name: str, environment: Optional[str] = None) -> None:
    ...
    if console_logging:
        ...
        fmt = (
            ...
            .get("default", {})
            .get("format", "[%(component)s] | %(module)s | %(levelname)s | %(message)s")
        )
...
class InterceptHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        """
        # Get the appropriate logger to use
        logger_name = "uvicorn" if record.name.startswith("uvicorn") else record.name
        if "." in logger_name:
        ...
        # Create a new record with the same properties
        ...
        # Add any extra attributes from the original record
        for key, value in record.__dict__.items():
            if key not in new_record.__dict__:
                new_record.__dict__[key] = value

        # Process the record through our handler chain
        for handler in logger.handlers:
            if handler != self:  # Avoid infinite recursion
...
def configure_uvicorn_logging(component_name: str) -> None:
    ...
    # Also intercept asyncio and other related loggers
