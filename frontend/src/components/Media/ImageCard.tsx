...
/**
 * Renders an interactive image card with hover actions, selection state, tags, and a context menu.
 *
 * Renders the provided image inside an aspect-ratio container and shows hover controls for favouriting.
 * The card displays a selection ring when selected, overlays image tags, and exposes a right-click context
 * menu with actions to favourite/unfavourite, copy the image to clipboard, and view image info.
 *
 * @param image - Image object to render (thumbnailPath, path, id, tags, isFavourite, etc.)
 ...
 * @param showTags - Whether to display the image's tags overlay
 * @param onClick - Optional click handler invoked when the card is clicked
 * @param imageIndex - Optional index of the image used when calling onViewInfo
 * @param onViewInfo - Optional handler invoked with (image, imageIndex) when "View Info" is selected; falls back to `onClick` if not provided
 * @returns A JSX element representing the interactive image card
 */
