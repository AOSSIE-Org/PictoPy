    ...
    >
      ...
      <div className="bg-muted relative h-48 w-full overflow-hidden">
        <img
          src={thumbnailSrc}
          alt="Weekend memory"
          onError={(e) => { e.currentTarget.src = '/photo.png'; }}
      ...
      <div className="space-y-1 p-4">
        <h3 className="line-clamp-1 text-lg font-semibold">Happy Weekends 🎉</h3>
        <p className="text-muted-foreground line-clamp-1 text-sm">{dateLabel}</p>
      </div>
    </div>
  );
};
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="bg-background border-b px-6 py-3">
        <div className="mb-3">
          <button
            onClick={onClose}
            className="flex cursor-pointer items-center gap-2 rounded-md border px-3 py-1.5 text-sm font-medium transition-colors hover:bg-secondary"
          >
        ...
        <div>
          <h1 className="flex items-center text-xl font-semibold text-gray-800 dark:text-gray-200">
            ...
            Happy Weekends 🎉
            <div className="mt-1 ml-2 text-sm font-normal text-gray-500">{dateLabel}</div>
          </h1>
        </div>
...
export const MemoriesPage: React.FC = () => {
  ...
  const weeklyMemories: WeeklyMemory[] =
    (weeklyMemoriesQuery.data as any)?.weekly_memories || [];
  ...
  const [filter, setFilter] = useState<'all' | 'location' | 'date' | 'weekends'>('all');

  // Selected weekend memory for detail view
  const [selectedWeekend, setSelectedWeekend] = useState<WeeklyMemory | null>(null);
          ...
          {hasAnyData && (
            <div className="mb-6 flex flex-wrap justify-start gap-2">
              ...
              <button
                ...
                className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${filter === 'weekends'
                  ? 'bg-foreground text-background'
                  : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                  }`}
              >
