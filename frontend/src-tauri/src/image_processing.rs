...
use std::sync::Arc;
use crate::cache::{IMAGE_CACHE, TOTAL_PROCESSING_TIME, PROCESSING_COUNT};
...
pub fn adjust_brightness_contrast(img: &DynamicImage, brightness: i32, contrast: i32) -> DynamicImage {
    ...
    // Generate cache key
    let cache_key = generate_cache_key(img, brightness, contrast);
    ...
    let lut = BrightnessContrastLUT::new(brightness, contrast);
    ...
    output.par_chunks_mut(3 * width as usize)
        .enumerate()
        .for_each(|(y, row)| {
            for x in 0..width {
                ...
                row[idx] = lut.transform(pixel[0]);
                row[idx + 1] = lut.transform(pixel[1]);
                row[idx + 2] = lut.transform(pixel[2]);
            }
        });
...
pub fn adjust_vibrance(img: &DynamicImage, vibrance: i32) -> DynamicImage {
    ...
    // Generate cache key
    let cache_key = format!("vibrance_{}_{}x{}", 
                           vibrance, img.width(), img.height());
    ...
    vibrant.par_chunks_mut(3)
        .for_each(|pixel| {
            ...
            let saturation = if max_val > 0.0 { 1.0 - (luma / max_val) } else { 0.0 };
...
pub fn adjust_temperature(img: &DynamicImage, temperature: i32) -> DynamicImage {
    ...
    // Generate cache key
    let cache_key = format!("temp_{}_{}x{}", 
                           temperature, img.width(), img.height());
...
pub fn apply_sharpening(img: &DynamicImage, amount: i32) -> DynamicImage {
    ...
    // Generate cache key
    let cache_key = format!("sharp_{}_{}x{}", 
    ...
    for y in 1..(height-1) {
        for x in 1..(width-1) {
            for c in 0..3 {
                ...
                let new_val = center * center_weight + 
                              (top + bottom + left + right) * neighbor_weight;
