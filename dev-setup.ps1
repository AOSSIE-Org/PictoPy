...
if (Get-Command node -ErrorAction SilentlyContinue) {
    ...
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) { Fail "Node.js installed but not found in PATH. Please restart PowerShell and re-run." }
...
if (Get-Command cargo -ErrorAction SilentlyContinue) {
    ...
    if (-not (Get-Command cargo -ErrorAction SilentlyContinue)) {
        Fail "Rust installed but cargo not in PATH. Please restart PowerShell and re-run."
    }
...
if ($hasBuildTools) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Visual Studio Build Tools installation failed." }
...
if (Test-Path $webview2Key) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "WebView2 Runtime installation failed." }
...
if (Get-Command conda -ErrorAction SilentlyContinue) {
    ...
    if (-not (Get-Command conda -ErrorAction SilentlyContinue)) {
        Fail "Miniconda installed but conda not in PATH. Please restart PowerShell and re-run."
    }
...
if (Test-Path $BACKEND_ENV) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Failed to create backend conda environment." }
    Ok "Backend conda environment created"
}

Info "Backend: checking Python packages..."
conda run -p $BACKEND_ENV python -c "import fastapi" 2>$null
...
if (Test-Path $SYNC_ENV) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Failed to create sync microservice conda environment." }
    Ok "Sync microservice conda environment created"
}

Info "Sync microservice: checking Python packages..."
conda run -p $SYNC_ENV python -c "import fastapi" 2>$null
if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Sync microservice pip install failed." }
...
$backendJob = Start-Job -ScriptBlock {
    ...
    conda run -p $using:BACKEND_ENV fastapi dev --port 52123 2>&1 | ForEach-Object { "[BACKEND]   $_" }
}
$jobs += $backendJob
...
$syncJob = Start-Job -ScriptBlock {
    Set-Location $using:SYNC_DIR
    conda run -p $using:SYNC_ENV fastapi dev --port 52124 2>&1 | ForEach-Object { "[SYNC]      $_" }
}
$jobs += $syncJob
...
try {
    ...
    Warn "Backend did not respond — check [BACKEND] logs below"
    $allHealthy = $false
}
...
try {
    ...
    Warn "Sync microservice did not respond — check [SYNC] logs below"
    $allHealthy = $false
}

if (-not $allHealthy) {
    Warn "One or more services failed to start. Check logs below then press Ctrl+C to exit."
} else {
    Write-Host ""
    Write-Host "✓ PictoPy dev environment is up!" -ForegroundColor Green
        ...
        foreach ($job in $jobs) {
            ...
            if ($output) {
                foreach ($line in $output) {
                    if ($line.StartsWith('[BACKEND]'))  { Write-Host $line -ForegroundColor Green }
                    elseif ($line.StartsWith('[SYNC]')) { Write-Host $line -ForegroundColor Yellow }
